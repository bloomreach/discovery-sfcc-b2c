...
function beforeStep(parameters) {
    ...
    // Data Hub Item Collections has no "views" concept, so the priceAsView
    // multi-currency strategy (which emits value.views) cannot be represented and
    // would be silently dropped or rejected. Fail loudly before generating a feed.
    // priceAsAttr is the only supported multi-currency strategy under Data Hub mode.
    var libBlr = require('*/cartridge/scripts/bloomreach/lib/libBloomreach');
    var deliveryMode = libBlr.getPreference('ProductFeedDeliveryMode');
    var multiCurrency = libBlr.getPreference('MiltiCurrency');
    if (deliveryMode === 'DataHub' && multiCurrency && multiCurrency.value === 'priceAsView') {
        Logger.error('Data Hub delivery does not support the priceAsView multi-currency strategy '
            + '(Item Collections has no views concept). Use priceAsAttr instead.');
