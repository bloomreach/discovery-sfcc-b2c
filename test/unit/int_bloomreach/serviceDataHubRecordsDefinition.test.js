...
var defPath = path.join(
    process.cwd(),
    'cartridges/int_bloomreach/cartridge/scripts/bloomreach/services/serviceDataHubRecordsDefinition.js'
);

/**
 * Load serviceDataHubRecordsDefinition with stubbed dw dependencies, capturing
 * the config object passed to LocalServiceRegistry.createService so we can drive
...
describe('serviceDataHubRecordsDefinition', function () {
    ...
    it('enables executeOverride and streams the body via client.send', function () {
        var loaded = load({});
        loaded.init();

        assert.isTrue(loaded.captured.config.executeOverride);

        var sent = [];
        var fakeSvc = {
            client: {
                send: function (body) { sent.push(body); }
            }
        };
        var someBody = { __file: true };
        loaded.captured.config.execute(fakeSvc, someBody);

        assert.lengthOf(sent, 1);
        assert.strictEqual(sent[0], someBody);
    });
});
